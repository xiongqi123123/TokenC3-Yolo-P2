...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/train_tokenc3.log'),
        logging.StreamHandler()  # 同时输出到控制台
    ]
)

# 创建模型 - 使用正确的路径
model_config = 'ultralytics/cfg/models/v13/yolov13n_tokenc3.yaml'
...
logging.info(f"Starting tokenc3 training...")
...
results = model.train(
    ...
    batch=64,                 # 批次大小
    imgsz=640,                 # 输入图片尺寸
    device="2,3",                # 使用的GPU设备ID（如"0"、"0,1"等）
    ...
    name='tokenc3_NoMosaic_Kernel_1',                  # 实验名称
...
metrics = model.val('/home/qi.xiong/Dataset/UAV_Sheep/yolo_dataset/dataset.yaml')
logging.info(f"Validation metrics: {metrics}")

# 测试推理
test_image = "/home/qi.xiong/Dataset/UAV_Sheep/yolo_dataset/test/images/2019_08_storli1_0766_JPG_jpg.rf.4221c6cdc0584e15fadccd357d5861d4.jpg"
if os.path.exists(test_image):
    logging.info(f"Testing inference on: {test_image}")
    results = model(test_image)
    results[0].show()
    logging.info("Inference test completed!")
else:
    logging.warning(f"Test image not found: {test_image}") 