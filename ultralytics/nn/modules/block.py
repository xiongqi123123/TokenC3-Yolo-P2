...
class DynamicLocalMixer(nn.Module):
    """
    Dynamic Local Mixing Module (DLLM) inspired by TransXNet.
    
    Implements input-dependent depthwise convolution for enhanced local feature learning.
    Specially designed for small target detection in agricultural pest scenarios.
    """
    
    def __init__(self, dim, kernel_size=1, reduction=4, min_dim=8): # kernel_size = 3
        """
        Args:
            dim: Input/output feature dimension
            kernel_size: Kernel size for depthwise convolution
            reduction: Channel reduction ratio for weight generation
            min_dim: Minimum dimension for weight generation network
        """
        super().__init__()
        self.dim = dim
        ...
        self.weight_gen = nn.Sequential(
            ...
            nn.Conv2d(mid_dim, dim * kernel_size * kernel_size, 1, bias=False),
            nn.Sigmoid()
        )
        
        # Multi-scale local context enhancement for small targets
        self.local_context = nn.ModuleList([
            nn.Conv2d(dim, dim, 3, 1, 1, groups=dim, bias=False),  # 3x3 for immediate neighbors 
            nn.Conv2d(dim, dim, 5, 1, 2, groups=dim, bias=False),  # 5x5 for broader local context
        ])
        # self.local_context = nn.ModuleList([
        #     nn.Conv2d(dim, dim, 1, 1, 0, groups=dim, bias=False),  # 1x1点卷积
        #     nn.Conv2d(dim, dim, (1,3), 1, (0,1), groups=dim, bias=False),  # 水平非对称
        #     nn.Conv2d(dim, dim, (3,1), 1, (1,0), groups=dim, bias=False),  # 垂直非对称
        # ])
        
        # Feature fusion and enhancement
        self.fusion = nn.Conv2d(dim * 3, dim, 1, bias=False)  # original + 2 context branches
        self.norm = nn.BatchNorm2d(dim)
        self.act = nn.SiLU(inplace=True)
        
        # Small target enhancement branch
    ...
    def forward(self, x):
        """Forward pass with dynamic local mixing."""
        B, C, H, W = x.shape
        
        # Generate dynamic weights
        weights = self.weight_gen(x)  # [B, C*K*K, 1, 1]
        weights = weights.view(B, C, self.kernel_size, self.kernel_size)
        
        # Apply input-dependent depthwise convolution
        x_pad = F.pad(x, [self.padding] * 4)
        dynamic_out = []
        
        for b in range(B):
            # Extract patches for each sample in batch
            patches = F.unfold(x_pad[b:b+1], self.kernel_size, stride=1)  # [1, C*K*K, H*W]
            patches = patches.view(C, self.kernel_size * self.kernel_size, H * W)  # [C, K*K, H*W]
            
            # Apply dynamic weights
            w = weights[b].view(C, self.kernel_size * self.kernel_size, 1)  # [C, K*K, 1]
            output = (patches * w).sum(dim=1).view(1, C, H, W)  # [1, C, H, W]
            dynamic_out.append(output)
        
        dynamic_features = torch.cat(dynamic_out, dim=0)  # [B, C, H, W]
        
        # Multi-scale local context
        context_features = []
        for local_conv in self.local_context:
            context_features.append(local_conv(x))
        
        # Fusion of all features
        all_features = torch.cat([dynamic_features] + context_features, dim=1)
        fused_features = self.fusion(all_features)
        
        # Small target enhancement
        enhanced = self.small_target_enhance(fused_features)
        output = self.act(self.norm(fused_features + enhanced))
        
        return output


# class TokenC3(nn.Module):
#     """
#     TokenC3: CSP Bottleneck with Dynamic Local Mixing for Agricultural Pest Detection.
    
#     Integrates TransXNet's DLLM into YOLOv13's C3 structure for enhanced small target detection.
#     Specially optimized for agricultural pest and disease detection scenarios.
#     """

#     def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, use_dllm=True, dllm_ratio=0.5):
#         """
#         Args:
#             c1: Input channels
#             c2: Output channels  
...
#             e: Expansion ratio
#             use_dllm: Whether to use Dynamic Local Mixing Module
#             dllm_ratio: Ratio of channels to apply DLLM (for efficiency)
#         """
#         super().__init__()
#         c_ = int(c2 * e)  # hidden channels
...
#         # Enhanced bottleneck layers with optional DLLM
#         if use_dllm:
#             dllm_channels = int(c_ * dllm_ratio)
#             regular_channels = c_ - dllm_channels
            
#             # Split bottlenecks: some with DLLM, some regular for efficiency
#             self.bottlenecks_dllm = nn.ModuleList([
#                 TokenBottleneck(dllm_channels, dllm_channels, shortcut, g, use_dllm=True) 
#                 for _ in range(n)
#             ])
#             self.bottlenecks_regular = nn.ModuleList([
#                 Bottleneck(regular_channels, regular_channels, shortcut, g, k=((1, 1), (3, 3)), e=1.0)
#                 for _ in range(n)
#             ])
            
#             # Channel split and fusion
#             self.channel_split = True
#             self.dllm_channels = dllm_channels
#             self.regular_channels = regular_channels
#         else:
...
#     def forward(self, x):
#         """Forward pass through TokenC3."""
#         x1 = self.cv1(x)  # First branch through cv1
#         x2 = self.cv2(x)  # Second branch through cv2
        
#         if self.channel_split and self.use_dllm:
#             # Split x1 for DLLM and regular processing
#             x1_dllm = x1[:, :self.dllm_channels]
#             x1_regular = x1[:, self.dllm_channels:]
            
#             # Process through respective bottlenecks
#             for dllm_block, regular_block in zip(self.bottlenecks_dllm, self.bottlenecks_regular):
#                 x1_dllm = dllm_block(x1_dllm)
#                 x1_regular = regular_block(x1_regular)
            
#             # Concatenate processed features
#             x1_processed = torch.cat([x1_dllm, x1_regular], dim=1)
#         else:
#             # Standard processing
#             x1_processed = self.m(x1)
        
#         # Final fusion
#         return self.cv3(torch.cat((x1_processed, x2), 1))


class TokenBottleneck(nn.Module):
    """
    Enhanced Bottleneck with Dynamic Local Mixing Module.
    
    Specialized bottleneck layer that incorporates DLLM for better small target feature extraction.
    """

    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5, use_dllm=True):
        """Initialize TokenBottleneck with optional DLLM."""
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        if use_dllm:
            # Use Dynamic Local Mixing instead of standard convolution
            self.cv2 = nn.Sequential(
                DynamicLocalMixer(c_, kernel_size=k[1]),
                nn.Conv2d(c_, c2, 1, groups=g, bias=False),
                nn.BatchNorm2d(c2),
                nn.SiLU(inplace=True)
            )
        else:
    ...
    def forward(self, x):
        """Forward pass through TokenBottleneck."""
        if isinstance(self.cv2, nn.Sequential):
            # DLLM path
            features = self.cv1(x)
            enhanced_features = self.cv2(features)
        else:
            # Standard path
            enhanced_features = self.cv2(self.cv1(x))
        
        return x + enhanced_features if self.add else enhanced_features


class GlobalLocalFusion(nn.Module):
    """
    Optional DGLM-inspired Global-Local Fusion module for Neck enhancement.
    
    This can be used later for long-range token interaction in the neck.
    """
    
    def __init__(self, c1, c2, num_heads=8, window_size=7):
        """
        Args:
            c1: Input channels
            c2: Output channels
        ...
        # Global branch (efficient attention)
        self.global_attn = nn.MultiheadAttention(c1, num_heads, batch_first=True)
        
        # Local branch (windowed convolution)
        self.local_conv = nn.Sequential(
            nn.Conv2d(c1, c1, window_size, 1, window_size//2, groups=c1),
            nn.BatchNorm2d(c1),
            nn.SiLU(inplace=True)
        )
        
        # Fusion
        self.fusion = Conv(c1 * 2, c2, 1, 1)
    
    def forward(self, x):
        """Forward pass with global-local fusion."""
        B, C, H, W = x.shape
        
        # Global attention
        x_flat = x.flatten(2).transpose(1, 2)  # [B, H*W, C]
        global_out, _ = self.global_attn(x_flat, x_flat, x_flat)
        global_features = global_out.transpose(1, 2).view(B, C, H, W)
        
        # Local convolution
        local_features = self.local_conv(x)
        
        # Fusion
        fused = torch.cat([global_features, local_features], dim=1)
        return self.fusion(fused)


class LSConv(nn.Module):
    """
    LSNet-inspired Large-Small Convolution for "See Large, Focus Small" strategy.
    
    Combines large kernel for global perception and small kernel for local focus,
    specially designed for UAV aerial view dense small target detection.
    """
    
    def __init__(self, c1, c2, large_kernel=7, small_kernel=3, groups=1, act=True):
        """
        Args:
            c1: Input channels
            c2: Output channels
            large_kernel: Large kernel size for global perception
            small_kernel: Small kernel size for local focus
            groups: Groups for grouped convolution
            act: Activation function
        """
        super().__init__()
        
        # Ensure all parameters are integers and valid
        c1, c2 = int(c1), int(c2)
        large_kernel, small_kernel = int(large_kernel), int(small_kernel)
        groups = max(1, int(groups))
        
        # Ensure groups compatibility with channels
        if c1 % groups != 0:
            groups = 1  # Fallback to groups=1 if incompatible
        
        # Ensure proper channel split for odd numbers
        self.c_large = c2 // 2
        self.c_small = c2 - self.c_large  # Handle odd numbers correctly
        
        # Ensure split channels are compatible with groups
        if self.c_large % groups != 0 or self.c_small % groups != 0:
            groups = 1  # Fallback to groups=1 for safety
        
        # Large kernel branch for "See Large" - global context
        self.large_branch = nn.Sequential(
            nn.Conv2d(c1, self.c_large, large_kernel, 1, large_kernel // 2, groups=groups, bias=False),
            nn.BatchNorm2d(self.c_large),
            nn.SiLU(inplace=True) if act else nn.Identity()
        )
        
        # Small kernel branch for "Focus Small" - local details
        self.small_branch = nn.Sequential(
            nn.Conv2d(c1, self.c_small, small_kernel, 1, small_kernel // 2, groups=groups, bias=False),
            nn.BatchNorm2d(self.c_small),
            nn.SiLU(inplace=True) if act else nn.Identity()
        )
        
        # 针对单类小目标优化：平衡大小核，强化小目标特征
        self.fusion_weight = nn.Parameter(torch.tensor([0.4, 0.6]))  # 更倾向小核处理小目标
        
        # 小目标专用注意力机制
        mid_channels = max(4, (self.c_large + self.c_small) // 8)
        self.small_target_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(self.c_large + self.c_small, mid_channels, 1),
            nn.ReLU(inplace=True),
        ...
        self.small_target_enhance = nn.Sequential(
            # 边缘检测增强小目标轮廓
            nn.Conv2d(self.c_small, self.c_small, 3, 1, 1, groups=self.c_small, bias=False),
            nn.BatchNorm2d(self.c_small),
            nn.ReLU(inplace=True),
            # 点特征增强
            nn.Conv2d(self.c_small, self.c_small, 1, 1, 0, bias=False),
            nn.BatchNorm2d(self.c_small),
            nn.Sigmoid()  # 用Sigmoid突出显著特征
        )
        
        # 背景抑制机制
        bg_mid_channels = max(1, self.c_large // 16)
        self.background_suppress = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(self.c_large, bg_mid_channels, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(bg_mid_channels, self.c_large, 1),
            nn.Sigmoid()
        )
        # Ensure fusion conv input matches concatenated channels
        concat_channels = self.c_large + self.c_small
        self.fusion_conv = Conv(concat_channels, c2, 1, 1, act=act)
        
    def forward(self, x):
        """单类小目标专用的See Large Focus Small策略."""
        large_features = self.large_branch(x)  # 背景上下文
        small_features = self.small_branch(x)  # 小目标细节
        
        # 🎯 针对单类小目标的专门处理
        
        # 1. 背景抑制 - 减少大核分支的背景干扰
        background_weights = self.background_suppress(large_features)
        suppressed_large = large_features * (1.0 - background_weights * 0.3)  # 轻度抑制背景
        
        # 2. 小目标特征增强 - 突出小核分支的目标特征  
        enhanced_small = self.small_target_enhance(small_features)
        boosted_small = small_features * (1.0 + enhanced_small * 0.5)  # 增强小目标
        
        # 3. 自适应融合权重计算
        combined = torch.cat([suppressed_large, boosted_small], dim=1)
        attention_weights = self.small_target_attention(combined)  # [B, 2, 1, 1]
        
        # 4. 单类小目标优化的特征融合
        # 大核：提供稳定的背景上下文，权重适中
        context_features = suppressed_large * attention_weights[:, 0:1, :, :]
        
        # 小核：提供关键的小目标特征，权重更高
        target_features = boosted_small * (attention_weights[:, 1:2, :, :] * 1.8)  # 强化小目标分支
        
        # 5. 残差连接确保稳定性
        final_large = context_features + 0.2 * large_features  # 保留原始上下文
        final_small = target_features + 0.4 * small_features   # 强化小目标路径
        
        # 6. 最终融合
        final_combined = torch.cat([final_large, final_small], dim=1)
        
        return self.fusion_conv(final_combined)


class LSGhostBottleneck(nn.Module):
    """
    LSNet + Ghost fusion bottleneck for maximum efficiency.
    
    Combines LSNet's large-small strategy with Ghost convolution's efficiency,
    optimized for high-density small target detection in UAV scenarios.
    """
    
    def __init__(self, c1, c2, large_k=5, small_k=3, shortcut=True, e=0.5, use_dllm=True):
        """
        Args:
            c1: Input channels
            c2: Output channels
            large_k: Large kernel size
            small_k: Small kernel size
            shortcut: Whether to use shortcut connection
            e: Expansion ratio
            use_dllm: Whether to integrate DLLM
        """
        super().__init__()
        
        # Ensure all parameters are correct types
        c1, c2 = int(c1), int(c2)
        large_k, small_k = int(large_k), int(small_k)
        
        # Simplified approach: use standard Conv instead of LSConv for internal processing
        # to avoid complex channel calculations in Ghost layers
        self.cv1 = Conv(c1, c2, 1, 1, act=True)  # Direct mapping
        
        # Use regular convolution instead of Ghost for stability
        self.cv2 = Conv(c2, c2, large_k, 1, act=True)
        
        # 单类小目标专用的动态局部混合
        if use_dllm:
            self.dllm = DynamicLocalMixer(int(c2), kernel_size=3, reduction=4)  # 专注小目标，适中设置
            
            # 单类小目标专用增强
            self.single_class_enhance = nn.Sequential(
                # 小目标边缘增强
                nn.Conv2d(int(c2), int(c2), 3, 1, 1, groups=int(c2), bias=False),
                nn.BatchNorm2d(int(c2)),
                nn.ReLU(inplace=True),
                # 特征聚合
                nn.Conv2d(int(c2), int(c2), 1, bias=False),
                nn.BatchNorm2d(int(c2)),
                nn.Sigmoid()  # 生成mask突出小目标区域
            )
            self.use_dllm = True
        else:
            self.use_dllm = False
    ...
    def forward(self, x):
        """单类小目标专用的LSGhostBottleneck前向传播."""
        # 标准卷积处理
        features = self.cv1(x)
        processed_features = self.cv2(features)
        
        # 单类小目标专用的DLLM处理
        if self.use_dllm:
            # 动态局部混合提取小目标特征
            dllm_features = self.dllm(processed_features)
            
            # 单类小目标mask生成与增强
            target_mask = self.single_class_enhance(processed_features)
            
            # 应用mask突出小目标区域
            masked_features = processed_features * target_mask
            enhanced_dllm = dllm_features * target_mask
            
            # 融合策略：基础特征 + 动态混合 + 掩码增强
            output = (processed_features + 
                     0.4 * enhanced_dllm +      # 增强的动态局部混合
                     0.3 * masked_features)     # 掩码增强的目标特征
        else:
            output = processed_features
...
class TokenC3(nn.Module):
    """
    TokenC3 Enhanced with LSNet "See Large, Focus Small" Strategy.
    
    This is the first innovation: combining TokenC3's dynamic local mixing
    with LSNet's large-small kernel strategy for optimal UAV small target detection.
    Maintains computational efficiency while enhancing feature representation.
    """
    
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, 
                 large_kernel=7, small_kernel=3, use_dllm=True, ls_ratio=0.6):
        """
        Args:
            c1: Input channels
            c2: Output channels
            ...
            large_kernel: Large kernel size for global perception
            small_kernel: Small kernel size for local focus
            use_dllm: Whether to use Dynamic Local Mixing
            ls_ratio: Ratio of channels for LSNet processing
        """
        super().__init__()
        
        # Ensure all parameters are correct types
        c1, c2, n = int(c1), int(c2), int(n)
        g = max(1, int(g))  # Ensure groups is positive
        large_kernel, small_kernel = int(large_kernel), int(small_kernel)
        
        c_ = int(c2 * e)  # hidden channels
        
        # Input processing with LSNet strategy
        self.cv1 = LSConv(c1, c_, large_kernel, small_kernel, g, act=True)
        self.cv2 = Conv(c1, c_, 1, 1)  # Standard branch for comparison
        self.cv3 = Conv(2 * c_, c2, 1)  # Final fusion
        
        # Enhanced bottleneck layers - ensure proper channel distribution
        ls_channels = int(c_ * ls_ratio)
        regular_channels = c_ - ls_channels
        
        # Ensure channels are divisible by groups
        if g > 1:
            # Make sure both channel counts are divisible by groups
            ls_channels = (ls_channels // g) * g
            regular_channels = c_ - ls_channels
            
            # If still not divisible, adjust
            if regular_channels % g != 0:
                regular_channels = (regular_channels // g) * g
                ls_channels = c_ - regular_channels
            
            # Final safety check
            if ls_channels <= 0:
                ls_channels = g
                regular_channels = c_ - g
            elif regular_channels <= 0:
                regular_channels = g
                ls_channels = c_ - g
        else:
            # For g=1, ensure at least 1 channel each
            if ls_channels == 0:
                ls_channels = 1
                regular_channels = c_ - 1
            elif regular_channels == 0:
                regular_channels = 1
                ls_channels = c_ - 1
        
        # Debug print to verify channel allocation
        # print(f"Debug: c_={c_}, ls_channels={ls_channels}, regular_channels={regular_channels}, g={g}")
        # print(f"Debug: ls_channels%g={ls_channels%g}, regular_channels%g={regular_channels%g}")
        
        # LSNet-enhanced bottlenecks for main processing
        self.ls_bottlenecks = nn.ModuleList([
            LSGhostBottleneck(ls_channels, ls_channels, large_kernel, small_kernel, 
                            shortcut, e=1.0, use_dllm=use_dllm)
            for _ in range(n)
        ])
        
        # Regular bottlenecks for efficiency balance
        self.regular_bottlenecks = nn.ModuleList([
            Bottleneck(regular_channels, regular_channels, shortcut, g, k=((1, 1), (3, 3)), e=1.0)
            for _ in range(n)
        ])
        ...
        self.cross_interaction = nn.Sequential(
            nn.Conv2d(c_, c_, 1, groups=c_, bias=False),
            nn.BatchNorm2d(c_),
            nn.SiLU(inplace=True)
        )
        
    def forward(self, x):
        """Forward pass through TokenC3_LSNet."""
        # Dual-branch processing
        x1 = self.cv1(x)  # LSNet-enhanced branch
        x2 = self.cv2(x)  # Standard branch
        
        # Split x1 for LSNet and regular processing
        x1_ls = x1[:, :self.ls_channels]
        x1_regular = x1[:, self.ls_channels:]
        
        # Process through respective bottlenecks
        for ls_block, regular_block in zip(self.ls_bottlenecks, self.regular_bottlenecks):
            x1_ls = ls_block(x1_ls)
            x1_regular = regular_block(x1_regular)
        
        # Concatenate and apply cross-interaction
        x1_processed = torch.cat([x1_ls, x1_regular], dim=1)
        x1_processed = self.cross_interaction(x1_processed)
        
        # Final fusion
        return self.cv3(torch.cat((x1_processed, x2), 1))


# Original TokenC3 (commented for reference and comparison)
# class TokenC3(nn.Module):
#     """
#     TokenC3: CSP Bottleneck with Dynamic Local Mixing for Agricultural Pest Detection.
#     
...
# Use the new TokenC3_LSNet as the main TokenC3 implementation
# TokenC3 = TokenC3_LSNet  # Modified Version
